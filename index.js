...
const generateItemElement = function (item) {
  ...
  if ((!item.checked) && (item.editable === false)) {
    itemTitle = `
     <span class='shopping-item'>${item.name}</span>
    `;
  } else if ((!item.checked) && (item.editable === true)){
    itemTitle = `
    <span class='shopping-item' contentEditable="true">${item.name}</span>
   `;
  }
  ...
  return `
    <li class='js-item-element' data-item-id='${item.id}' >
      ...
      <div class='shopping-item-controls'>
        ...
        <button class="edit-item js-item-edit">
...
const addItemToShoppingList = function (itemName) {
  store.items.push({ id: cuid(), name: itemName, checked: false, editable: false });
};
...
const handleItemEdit = function () {
  ...
  /*for (var i = 0; i < store.items.length; i++) {
    localStorage.setItem(store.items[i].name, store.items[i].innerHTML);*/
  
}

const editItemButton = function (id) {
  const index = store.items.findIndex(item => item.id === id);
  store.items[index].editable = !store.items[index].editable;
  console.log(store.items[index].editable)
